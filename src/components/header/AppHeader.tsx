...
import EmpresaDisplayer from "./EmpresaDisplayer";
...
export default function AppHeader() {
  const { sentinelRef, intersecting } = useIntersectionObserver({
    root: null,
    threshold: 1,
  });
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-20 h-16 bg-paper/80 flex items-center border-b border-border backdrop-blur-lg drop-shadow-sm">
      ...
      <EmpresaDisplayer ref={sentinelRef} />
    </>
  );
}
